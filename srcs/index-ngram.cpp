...
// Divides one ordered n-gram file to index files.
class index_divider
{
public:
	typedef long long int64;
	// Unigram ID and n-gram ID.
	typedef pair<int, int> pair_type;

	index_divider(int n, int memory_size, const string &output_prefix)
		: n_(n), output_prefix_(output_prefix), max_id_(0),
		freqs_(), unigrams_(), pairs_(), total_count_(0), file_id_(0)
	{
		// Allocates memory to buffers.
		max_id_ = (int)(((int64)memory_size << 20)
			/ (sizeof(int64) + sizeof(int) * n_ + sizeof(pair_type) * n_));
		freqs_.resize(max_id_);
		unigrams_.resize(max_id_ * n_);
		pairs_.resize(max_id_ * n_);
	}
...
private:
	...
	index_divider(const index_divider &);
	index_divider &operator=(const index_divider &);
...
private:
	bool fill(ngram::vbe_reader &reader)
	{
		...
		for (int id = 0; id < max_id_; ++id)
		{
			...
			for (int i = 0; i < n_; ++i)
			{
				...
				pairs_.push_back(make_pair(unigram, id));
			}
	...
	bool flush(vector<string> *output_file_names)
	{
		...
		pairs_.erase(unique(pairs_.begin(), pairs_.end()), pairs_.end());
...
public:
	typedef long long int64;
	// Unigram ID and file ID.
	typedef pair<int, int> pair_type;

	index_merger(int n)
		: n_(n), input_files_(), byte_readers_(), readers_(), queue_() {}

	bool merge(ostream &index_output, ostream &data_output,
		const vector<string> &input_file_names)
	{
		...
		if (!merge_files(index_output, data_output))
			return false;
...
private:
	...
	bool init_queue()
	{
		for (size_t i = 0; i < input_files_.size(); ++i)
		{
			...
			*readers_[i] >> unigram;
			...
			queue_.push(make_pair(unigram, static_cast<int>(i)));
		}
		return true;
	}

	// Merges input files.
	bool merge_files(ostream &index_output, ostream &data_output)
	{
		ngram::byte_writer index_byte_writer(index_output);
		ngram::vbe_writer index_writer(index_byte_writer);

		ngram::byte_writer data_byte_writer(data_output);
		ngram::vbe_writer data_writer(data_byte_writer);

		int prev_unigram_id = 0;
		int64 prev_address = 0;
		int64 count = 0;
		while (!queue_.empty())
		{
			...
			int unigram_id = pair.first;
			int file_id = pair.second;

			// Writes delimitor.
			if (unigram_id != prev_unigram_id)
			{
				data_writer << 0;

				int64 current_address = data_byte_writer.count();
				index_writer << (unigram_id - prev_unigram_id);
				index_writer << (current_address - prev_address);

				prev_unigram_id = unigram_id;
				prev_address = current_address;
			}

			// Copies n-gram.
			copy_ngram(data_writer, file_id);

			// Reads unigram ID of the next n-gram.
			*readers_[file_id] >> unigram_id;
			if (unigram_id)
				queue_.push(make_pair(unigram_id, file_id));

			if (!(++count % 100000))
				cerr << "merging... " << count << '\r';
		}
		index_writer << 0;
		index_writer << (data_byte_writer.count() - prev_address);
	...
	// Copies n-gram.
	bool copy_ngram(ngram::vbe_writer &data_writer, int file_id)
	{
		// Copies frequency.
		int64 freq;
...
int main(int argc, char *argv[])
{
	...
	{
		cerr << "Usage: " << argv[0]
			<< " N MemorySize IndexFile TempFilePrefix" << endl;
		return 1;
	}
	...
	const string index_file_name = argv[3];
	...
	const string temp_file_prefix = argv[4];
	...
	{
		// Divides one ordered n-gram file to index files.
		index_divider divider(n, memory_size, temp_file_prefix);
	...
	{
		// Merges index files into one index file.
		index_merger merger(n);
		if (!merger.merge(index_file, cout, temp_file_names))
			return 1;
	}
