...
// Filters out.
bool Filter(const ssgnc::Ngram &ngram, const std::vector<int> &filters)
{
	if (filters.empty())
		return true;

	unsigned mask = 0;
	for (int i = 0; i < ngram.unigram_size(); ++i)
	{
...
int main(int argc, char *argv[])
{
	...
	// Opens a vocabulary.
	ssgnc::FileMapper vocab_file;
	if (!OpenFile(path_generator.VocabPath(), &vocab_file))
		return 1;
	ssgnc::VocabIndex vocab(vocab_file);
	...
	for (int i = 1; ; ++i)
	{
		...
		boost::shared_ptr<ssgnc::TextFinder>
			text_finder(new ssgnc::TextFinder(*text_file, i));
	...
	while (std::getline(std::cin, query_string))
	{
		...
		for (std::size_t n = tuples.size(); n <= index_finders.size(); ++n)
		{
			...
			for (std::size_t i = 1; i < tuples.size(); ++i)
			{
				...
				{
					if (filters.empty() || filters.back() != tuples[i].key_id)
						filters.push_back(tuples[i].key_id);
					filters.push_back(tuples[i].key_id);
					continue;
				}
			...
			{
				if (!text_finders[n - 1]->Find(positions[i], &ngram))
				{
